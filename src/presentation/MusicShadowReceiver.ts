import {
  musicSnapshotDifferences,
  type MusicSlide,
  type MusicSnapshot,
} from "./MusicPresentationCore";
...
export function readMusicShadowPacket(value: unknown): MusicShadowPacket | null {
  ...
  if (
    ...
    !s.sessionId.length ||
    s.sessionId.length > 128 ||
    !counter(s.revision) ||
    typeof s.active !== "boolean" ||
    typeof s.title !== "string" ||
    s.title.length > 4096 ||
    !counter(s.slideIndex) ||
    !counter(s.totalSlides)
  )
    return null;
  ...
  if (
    s.active &&
    (s.totalSlides === 0 ||
      s.slideIndex >= s.totalSlides ||
      slide === null ||
      s.slideIndex + 1 < s.totalSlides !== (nextSlide !== null))
  )
    return null;
  return {
    version: 1,
...
type LegacySelection = Pick<
  MusicSnapshot,
  "title" | "slideIndex" | "totalSlides" | "slide" | "nextSlide" | "active"
>;
...
export class MusicShadowReceiver {
  ...
  receive(value: unknown): void {
    ...
    if (
      this.packet?.snapshot.sessionId === packet.snapshot.sessionId &&
      (packet.snapshot.revision < this.packet.snapshot.revision ||
        packet.legacyRevision < this.packet.legacyRevision)
    )
      return;
    this.packet = packet;
  }
  ...
  observe(sessionId: unknown, revision: unknown, selection: LegacySelection): void {
    ...
    if (typeof sessionId !== "string" || !counter(revision)) {
      this.legacy = null;
      return;
    }
  ...
  takeDifferences(): string[] {
    ...
    if (
      ...
      !packet ||
      !legacy ||
      packet.snapshot.sessionId !== legacy.sessionId ||
      packet.legacyRevision !== legacy.revision ||
      packet.snapshot.active !== legacy.selection.active ||
      this.reportedSession === legacy.sessionId
    )
      return [];
  ...
  snapshot(): MusicSnapshot | null {
    return this.packet?.snapshot ?? null;
  }
}
