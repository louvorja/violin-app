...
function ud<T>(key: string, fallback: T): T {
  void tick.value;
  const manifestDefault = moduleCustomizationDefault(ModuleEnum.NAME_DRAW, key, fallback);
  const v = UserData.get<T>(`modules.name_draw.${key}`, manifestDefault);
...
/* Pílula com medidas em em: a fonte do chip é escolhida pelo usuário e
   reescalada pelo tamanho do container, então altura fixa cortaria o nome. */
.ndraw-proj-chip {
  display: inline-flex;
  align-items: center;
  margin: 5px;
