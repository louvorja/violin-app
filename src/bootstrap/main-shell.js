...
$storage.hydrate().then(async () => {
  ...
  createI18nInstance(UserData.get(KEYS.OPTIONS.LANGUAGE)).then(async (i18n) => {
    ...
    // e só começa a fila depois de dois frames já renderizados.
    const bootOrchestrator = new BootOrchestrator({
      ...
      onTaskError: (task, error) => {
        console.warn(`[main] tarefa de boot "${task.id}" falhou:`, error);
        Telemetry.captureException(error, {
          source: "boot_maintenance",
          task_id: task.id,
        });
      },
    });
    ...
    if (!isAuxiliaryRenderer) {
      void bootOrchestrator
        .afterFirstPaint([
          ...
          {
            id: "scheduled_store_hydrate",
            run: () => ScheduledStore.hydrate(),
          },
          {
            id: "liturgy_migration",
            run: async () => {
              const legacyItems = UserData.get("modules.liturgy.items");
        ...
        .then(() => _bootStage("post_paint_maintenance_ready"));

      bootOrchestrator.idleMaintenance([]);
    } else {
      void bootOrchestrator.afterFirstPaint();
    }
