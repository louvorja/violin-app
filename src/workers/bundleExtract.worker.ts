...
async function extract({ kind, buffer }: ExtractRequest): Promise<void> {
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    const key = kind === "bible" ? path.split("/").at(-1)!.replace(/\.json$/, "") : catalogKey(path);
    if (!key) continue;
    if (seen.has(key)) throw new Error(`Bundle inválido: chave duplicada ${key}`);
    seen.add(key);
    const raw = await zip.files[path].async("text");
    const data = JSON.parse(raw) as unknown;
    self.postMessage({ type: "entry", key, data, current: index + 1, total: entries.length });
    await waitForContinue();
  }
  self.postMessage({ type: "done" });
}

self.onmessage = (event: MessageEvent<ExtractRequest | { type: "continue" }>) => {
